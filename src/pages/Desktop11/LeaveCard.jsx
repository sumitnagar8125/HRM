...
export default function LeaveCard({ leave }) {
  // 🛡️ Safe defaults to avoid undefined crashes
  ...
  return (
    <div className="flex items-center gap-5 bg-gradient-to-r from-blue-50 to-white rounded-xl p-6 mb-4 shadow border border-sky-100 relative group">
      <div className="shrink-0">{leaveTypeIcon(type)}</div>
      <div className="flex-1 min-w-0">
        <div className="flex flex-wrap items-center gap-2 mb-1">
          <span className="text-lg font-bold text-sky-900 uppercase">
            {type}
          </span>
          <span className="text-sm text-sky-700 font-semibold">
            {prettyDate(start_date)} &rarr; {prettyDate(end_date)}
          </span>
        </div>
        {employee_id && (
      ...
      <div className="ml-2 flex flex-col items-end gap-2">
        <span
          className={`rounded-full px-4 py-1 uppercase text-xs tracking-wider font-bold ${statusClass(
            status
          )}`}
        >
          {status}
