...
export default function Calenderui({ selectedDate, setSelectedDate, role, maxPastDays = 14 }) {
  const today = startOfDay(new Date());
  const maxWeeks = Math.ceil(maxPastDays / 7);

  const [weekStart, setWeekStart] = useState(startOfWeek(today, { weekStartsOn: 0 }));
  const canGoBack = differenceInCalendarDays(weekStart, subWeeks(today, maxWeeks)) > 0;
  const canGoForward = differenceInCalendarDays(startOfWeek(addWeeks(weekStart, 1)), today) <= 0;
  const days = [...Array(7)].map((_, i) => addDays(weekStart, i));
  const isDisabled = (day) => isBefore(day, subWeeks(today, maxWeeks)) || isAfter(day, today);

  const onDateClick = (day) => {
    if (!isDisabled(day)) setSelectedDate(day);
  };
    ...
    <div>
      <div className="flex justify-between items-center mb-4">
        <button
          ...
          className={`text-xl font-bold ${canGoBack ? "text-gray-600 hover:text-blue-600" : "text-gray-300 cursor-not-allowed"}`}
          disabled={!canGoBack}
        >
          &lt;
        </button>
        ...
        <button
          ...
          className={`text-xl font-bold ${canGoForward ? "text-gray-600 hover:text-blue-600" : "text-gray-300 cursor-not-allowed"}`}
